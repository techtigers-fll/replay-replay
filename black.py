...
def run(robot: Robot):
    ...
    turn_pid = Pid(10, 0, 5)
    slow_turn_pid = Pid(5, 0, 0)
    drive_pid = Pid(1, 0, 0)
    robot.reset_sensors()

    robot.drive(drive_pid, 400, 0, 375) 
    robot.turn(slow_turn_pid, 55)
    robot.drive(drive_pid, 400, 55, 250) 
    robot.follow_line(straight_line_follow_pid, 400, 600, LineSensor.RIGHT, LineEdge.LEFT)
